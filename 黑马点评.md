...
![image-20250904194108457](E:\Typora picture\image-20250904194108457.png)

## 缓存击穿

![image-20250904195943186](E:\Typora picture\image-20250904195943186.png)

## 乐观锁和悲观锁

![image-20250906164837738](E:\Typora picture\image-20250906164837738.png)

## Redission实现分布式锁

redission是一个redis官方提供的工具，里面封装了很多好东西不需要我们自己去实现

官方文档：[Redisson分布式锁使用详解-阿里云开发者社区](https://developer.aliyun.com/article/1650839)
...
public class LuaScriptLoader {
    // 缓存：脚本名 -> Resource（不直接存 DefaultRedisScript，避免 resultType 冲突）
    private static final Map<String, Resource> SCRIPT_RESOURCE_CACHE = new HashMap<>();
    static {
        try {
            ...
            for (Resource resource : resources) {
                String name = resource.getFilename().replace(".lua", "");
                SCRIPT_RESOURCE_CACHE.put(name, resource);
            }
        } catch (IOException e) {
     ...
     * 获取脚本，并指定返回类型
     */
    public static <T> DefaultRedisScript<T> getScript(String name, Class<T> resultType) {
        Resource resource = SCRIPT_RESOURCE_CACHE.get(name);
        if (resource == null) {
            throw new IllegalArgumentException("未找到脚本: " + name);
        }
